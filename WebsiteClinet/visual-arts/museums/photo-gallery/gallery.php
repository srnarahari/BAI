...
<?php 
        ...
        // custom function to get array value which is enabled 
	       
        function getEnabledValue($passArray , $kia){
            foreach($passArray as $keyss => $value)
            {
                if($value == $kia)
                {
                   return $keyss;
	...
	foreach($slideshowArrays as $key => $slideshowArray):       // looping for slideshow li
          ...
          // values checked for gallery slider
          $SelectedCountry  = getEnabledValue($LifesytlesChannels[0],1);
          $SelectedCategory  = getEnabledValue($sub_channel[0],1);
      ...
      endforeach; // first loop for slideshow 
    ...
    <div class="col-lg-12 phototopSection">
      <div class="col-lg-2  hidden-sm hidden-xs">
        <div class="logo">
			...
			<p class="backto"><a href="<?php echo $path ?>visual-arts/slideshows.php"> ‹ RETURN TO ARTICLE</a></p>
        </div>
      </div>
